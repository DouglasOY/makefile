...
#define FIFO_PANIC 0x1A2B3C
#define GLOBALFIFO_MAJOR 211
...
static int douglas_add_one(int i)
{
    int r = 276;

    r += i;
    printk(KERN_INFO "douglas_add_one: [%d]\n", r);

    r += douglas_panic();
    return r;
}

static int douglas_add_eight(int a, int b, int c, int d, int e, int f, int g, int h)
{
    int r = 275;
    int i = 97;

    r += a + b + c + d + e + f + g + h; 
    printk(KERN_INFO "douglas_add_eight: [%d]\n", r);
    r += douglas_add_one(i);
    return r;
}

static int douglas_add_two(int first, int second)
{
    int r = 274;
    int a = 81; /* 0x51 */
    int b = 82;
    ...
    r += first + second;
    printk(KERN_INFO "douglas_add_two: [%d]\n", r);
    r += douglas_add_eight(a, b, c, d, e, f, g, h);
    return r;
}

static int douglas_main_init(void) 
{
    int r = 273; /* 0x111 */
    int first = 5;
    int second = 6;
    ...
    r += douglas_add_two(first, second);
    printk(KERN_INFO "douglas_main: [%d]\n", r);
			     ...
			     unsigned long arg)
{
	...
	case FIFO_PANIC:
		...
		printk(KERN_INFO "globalfifo is set to panic\n");
        douglas_main_init();
		break;
...
MODULE_AUTHOR("Douglas OY <do@kernel.org>");
